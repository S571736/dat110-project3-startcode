...
public class FileManager extends Thread {
    ...
    public boolean requestToReadFileFromAnyActiveNode(String filename) throws RemoteException, NotBoundException {
        ...
        Message nodeMessage = new ArrayList<>(active).get(0);
        // locate the registry and see if the node is still active by retrieving its remote object
        ChordNodeInterface node =
                (ChordNodeInterface) Util.locateRegistry(nodeMessage.getNodeIP()).lookup(nodeMessage.getNodeID().toString());
        ...
        node.setActiveNodesForFile(active);
        ...
        if (msg.isAcknowledged()) {
            ...
            Operations op = new Operations(node, nodeMessage, active);
            op.performOperation();
    ...
    public boolean requestWriteToFileFromAnyActiveNode(String filename, String newcontent) throws RemoteException, NotBoundException {
        ...
        // locate the registry and see if the node is still active by retrieving its remote object
        ChordNodeInterface node =
                (ChordNodeInterface) Util.locateRegistry(nodeMessage.getNodeIP().toString()).lookup(nodeMessage.getNodeID().toString());
        ...
        // perform operation by calling Operations class
            Operations op = new Operations(chordnode, msg, active);
            op.performOperation();
